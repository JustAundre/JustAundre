# The Amazing Shell *Syntax* Guide
...
>
> This covers the syntax for shell script. However, a majority of the function of a script comes from the actual commands, and by extension, a majority of shell script writing is based on remembering the commands for certain things.
...
# Figure 1
echo "Hello!" # "Hello!" is a normal argument.

# Figure 2
echo -n "Hello!" # Ditto, but with a "-n", which disables printing the leading newline.
...
Comprised of the initiator `if`, the result `then`, an exception `else`, and additional conditions `elif`, and the closing `fi`,
...
# A crime against humanity, thou who scripts like this shall be ensnared in the circles of hell.
...
> {
> 	# Print "Hello!"
> 	# The ">&2" redirects the "Hello!" to fd2/stderr, which will make it hidden,
> 	# due to the directive attached to the command group.
> 	echo "Hello!" >&2
> 
...
Ditto, but with the opening and closing parenthesis `()`, will do mostly the same thing as command groups but with the distinction that everything that goes on inside the sub-shell is ephemeral and will disappear when it finishes. As a result, the following effects can be observed, but note that this is not an exhaustive list:
...
> [!TIP]
>
> Pipes in this context, will actually create a sub-shell, meaning the assignment or reassignment of variables inside pipes will not persist into the parent shell.
...
Usually made by the `mkfifo`[^First in, first out] command, can live beyond the lifespan of an arbitrary process—any program can put data into it, and any program can read from it, but ***both must happen simultaneously*** because the data is not stored unlike a normal file.
